...
ASCII_ART = [
    r"   ______                          __",
    r"  / ____/___ _ ___  _____ ____ _ _/ /__",
    r" / /   / __ `// __|/ ___// __ `// __  /",
    r"/ /___/ /_/ / \__ \/ /__ / /_/ // /_/ / ",
    r"\____/\__,_/ /___/\___/ \__,_/ \__,_/  ",
    r"",
]

def render_banner() -> str:
    """Render gradient-colored ASCII art banner with version and info."""
    lines = []
    for i, line in enumerate(ASCII_ART):
        color = GRADIENT[i % len(GRADIENT)]
        lines.append(f"{BOLD}{color}{line}{RESET}")

    lines.append(f"  {DIM}v{VERSION} — HEP Agentic CLI Orchestrator{RESET}")
...
def render_status_bar(provider: str, model: str) -> str:
    """Render one-line model status bar."""
    return f"  {DIM}Model: {CYAN}{model}{DIM}  │  Provider: {CYAN}{provider}{RESET}"
```

**Step 2: Commit**
```bash
git add src/cascade/ui/banner.py
git commit -m "feat(ui): implement ASCII art banner with gradient rendering"
```
